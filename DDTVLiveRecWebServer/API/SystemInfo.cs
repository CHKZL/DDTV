...
namespace DDTVLiveRecWebServer.API
{
    ...
    {
        public static string Web(Dictionary<string,string> Form)
        {
            var 鉴权结果 = 鉴权.Authentication.API接口鉴权(Form);
            if (!鉴权结果.鉴权结果)
            {
                string mes = JsonConvert.SerializeObject(new Messge
                {
                    result = 鉴权结果.鉴权结果,
                    messge = 鉴权结果.鉴权返回消息
                });
            ...
            else
            {
                ...
                string mes = JsonConvert.SerializeObject(new Messge
                {
                    ...
                    systemInfo=new Messge.SystemInfo() 
                    {
                        ...
                        Download_Info=new Messge.Download_Info()
                        {
                            Downloading= 下载中,
                            Completed_Downloads= 下载完成
                        },
                        Ver_Info=new Messge.Ver_Info()
                        {
