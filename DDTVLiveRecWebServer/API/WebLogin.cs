    ...
    public class WebLogin
    {
        ...
        public static string Web(HttpContext context)
        {
            ...
            if(!string.IsNullOrEmpty(UserName) || !string.IsNullOrEmpty(Password))
            {
                if(是否初次登陆)
                {
                    下次可登陆时间 = DateTime.UtcNow;
                    是否初次登陆 = false;
                }
                if (DateTime.UtcNow.AddSeconds(3) > 下次可登陆时间)
                {
                    下次可登陆时间 = DateTime.UtcNow.AddSeconds(5);
                    ...
                    else
                    {
                        if(错误次数>=5)
                        {
                            下次可登陆时间 = DateTime.UtcNow.AddMinutes(30);
                            return JsonConvert.SerializeObject(new Auxiliary.RequestMessage.ServerClass.Login()
                            {
                                message = "验证失败,服务器拒绝鉴权操作10分钟",
                                result = false,
                            });
                        }
                        else
