                ...
                try
                {
                    if (Directory.Exists(Auxiliary.MMPU.缓存路径 + context.Request.Form["Directory"]))
                    {
                        if (File.Exists(Auxiliary.MMPU.缓存路径 + context.Request.Form["Directory"] + "\\" + context.Request.Form["Name"]))
                        {
                            ...
                            return ReturnInfoPackage.InfoPkak((int)ReturnInfoPackage.MessgeCode.请求成功, "请求成功", new List<FileInfo>() { new FileInfo() { result = true, messge = "文件已提加入删除委托列表，等待文件锁解锁后自动删除" } });
                        }
                        else
                        {
                            return ReturnInfoPackage.InfoPkak((int)ReturnInfoPackage.MessgeCode.请求成功, "请求成功", new List<FileInfo>() { new FileInfo() { result = false, messge = "该文件不存在" } });
                        }
                    }
                    else
                    {
                        return ReturnInfoPackage.InfoPkak((int)ReturnInfoPackage.MessgeCode.请求成功, "请求成功", new List<FileInfo>() { new FileInfo() { result = false, messge = "路径有误" } });
                    }
                }
                catch (Exception)
                {
                    return ReturnInfoPackage.InfoPkak((int)ReturnInfoPackage.MessgeCode.请求成功, "请求成功", new List<FileInfo>() { new FileInfo() { result = false, messge = "文件路径错误或者格式符合法！" } });
                }
            }
        }
