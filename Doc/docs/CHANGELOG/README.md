...
### Ver5.7.3（2026-07-25）
●　新增SEND_GIFT_V2礼物消息解析器：支持将新版protobuf格式的礼物消息解码为旧版SEND_GIFT数据结构，保证礼物消息正常录制  
●　新增观看时长心跳功能：在VLC播放窗口观看直播时可向B站发送观看心跳，记录观看时长  
●　重构心跳相关逻辑，优化性能和资源管理  

### Ver5.7.2（2026-07-20）
●　修复一揽子性能问题：巡检循环O(n²)及Timer无重入保护、房间查找优化为O(1)、get_rooms接口O(n²)、文件结构接口全量扫盘、日志页订阅泄漏、弹幕渲染磁盘IO与同步Invoke、房间卡片每秒全量diff、页面定时器泄漏、播放窗口UI线程同步HTTP、VLC资源不释放等  
●　日志数据库改为批量写入，重构WebSocket推送逻辑，提升IO性能与稳定性  
●　优化fMP4修复流程：支持复用扫描结果，减少重复扫描，并优化临时文件清理  
●　优化弹幕渲染逻辑：减少视觉树节点数、新增单元素描边文本，弹幕速度默认值调整为8  
●　新增一键关闭所有播放窗口功能  
●　播放失败增加连续失败计数，防止无限重连，失败时提示手动刷新  
●　修复托盘图标初始化可能内存泄漏的问题；改进录制历史同步机制，增强异常处理和重连逻辑  

### Ver5.6.14（2026-07-06）
●　新增直播间标题变更时自动切割录制文件功能  
●　新增桌面端窗口布局功能及相关管理逻辑  
●　重构FLV流地址获取逻辑，增强异常处理，避免获取到无效流导致录制错误  
●　录制状态管理等逻辑重构为异步方法，增强异常处理和用户提示  

### Ver5.6.10 ~ Ver5.6.11（2026-05-29）
●　重构FLV下载逻辑，移除Downloader组件依赖  
●　修复flv文件路径缺少扩展名的问题  
●　优化房间卡片同步逻辑与定时任务间隔  
●　优化数据页卡片排序逻辑  

### Ver5.6.3 ~ Ver5.6.8（2026-05-24）
●　新增fMP4修复功能，优化下载逻辑  
●　修复SMTP邮件推送问题，调整邮件默认发件人名称  
●　优化GUI交互逻辑，防止UI线程被阻塞导致无响应  
●　支持弹幕录制状态的处理；优化重连逻辑并修复事件绑定问题  
●　新版自动更新程序：支持动态并行数和已完成文件列表显示  
●　修复控制台光标越界、UI框架升级导致按钮字体过大等界面问题；优化下载文件命名规则  

### Ver5.5.8（2026-01-18）
●　【重要】运行框架升级到.NET 10  
●　增强HLS分辨率检测：触发PK等分辨率切换情况时自动切断并新建视频文件，防止花屏  
●　下载链路重构为HttpClient并优化重试机制  
●　新增按房间级别的文件大小/录制时长切割配置，新增对应的全局与房间维度切割查询和设置API  
●　新增媒体分片合并功能  
●　实现弹幕断线重连功能  
●　SC（醒目留言）增加消息内容显示  
●　桌面端新增日志管理、高级修复、工具页文件选择等功能；右键房间卡片可直接打开录播文件夹  

### Ver5.2.x（2025-01 ~ 2025-08）
●　Desktop与Core的交互从HTTP请求改为直接函数访问，提升响应速度  
●　新增SMTP邮件通知功能，开播、录制等事件可邮件推送  
●　新增EnableWebServer配置项用于控制是否启用WEB服务；CLI模式下Swagger可通过配置开关  
●　修复HLS任务偶尔卡在速度为0的问题；优化HLS降级为FLV的录制逻辑和网络波动时弹幕断连的问题  
●　重写托盘图标控件，新增托盘右键菜单；修复explorer.exe重启后托盘图标丢失的问题  
●　播放窗口弹幕功能增强：最大弹幕长度配置、兼容播放窗口、复制流地址、窗口置顶等  
●　发布包内置MKVToolnix插件；升级VLC组件  

### Ver5.0.x ~ Ver5.1.x（2024-05 ~ 2025-01）
●　DDTV5完全重构：拆分为Core/Server/Desktop/Client架构，WEBUI重构为React版本  
●　桌面端播放方案改为VLC方案，重构DDTV3时期的播放器和弹幕渲染功能  
●　新增自动更新程序；新增WebHook功能  
●　新增按文件大小、按录制时长切割录制文件功能  
●　新增按关注分组导入房间、录制历史查看、弹幕屏蔽词设置功能  
●　新增快照切片、排故调试快照、阻止系统休眠开关等功能  
●　支持付费直播间的录制尝试和修复  
●　Desktop增加兼容播放模式  
...
:::warning 归档说明
以下为 **DDTV3 及更早版本** 的历史更新记录，仅作归档保留，内容不再更新。DDTV5的更新记录请查看页面上半部分。  
:::
