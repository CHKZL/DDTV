                ...
                if(rlc.Token==webSocketConnection.Token)
                {
                    switch (rlc.code)
                    {
                        case (int)ClientSendMessageCode.获取系统运行情况:
                            return RequestMessage.封装消息.获取系统消息.系统消息();
                        case (int)ClientSendMessageCode.查看当前配置文件:
                            return RequestMessage.封装消息.获取配置文件信息.配置文件信息();
                        case (int)ClientSendMessageCode.检查更新:
                            return RequestMessage.封装消息.获取检查更新信息.检查更新信息();
                        ...
                        case (int)ClientSendMessageCode.获取当前录制中的队列简报:
                            return RequestMessage.封装消息.获取当前录制中的任务队列简报信息.当前录制中的任务队列简报信息();
                        case (int)ClientSendMessageCode.获取所有下载任务的队列简报:
                            return RequestMessage.封装消息.获取所有下载任务的简报队列信息.所有下载任务的简报队列信息();
                        ...
                        case (int)ClientSendMessageCode.获取当前房间配置列表总览:
                            return RequestMessage.封装消息.获取当前房间配置列表总览信息.当前房间配置列表总览信息();
                        case (int)ClientSendMessageCode.获取当前录制文件夹中的所有文件的列表:
                            return RequestMessage.封装消息.获取当前录制文件夹中的所有文件的列表信息.当前录制文件夹中的所有文件的列表信息();
                        ...
                        case (int)ClientSendMessageCode.获取上传中的任务信息列表:
                            return RequestMessage.封装消息.获取上传中的任务信息列表信息.上传中的任务信息列表信息();
