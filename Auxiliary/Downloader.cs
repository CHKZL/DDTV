        ...
        public static void 下载任务状态检测()
        {
            foreach (var item in MMPU.DownList)
            {
                if(!item.DownIofo.网络超时&& item.DownIofo.下载状态&&item.DownIofo.已下载大小bit>10000&& item.DownIofo.最后连接时间!=0&& Gettime()-item.DownIofo.最后连接时间>60)
                {
                    ...
                    up.UploadVideo(ref item.DownIofo, item.DownIofo.文件保存路径, MMPU.oneDrivePath + item.DownIofo.主播名称 + "_" + item.DownIofo.房间_频道号 + "/");
                }
            }
        }
                ...
                try
                {
                    ...
                    if (e.Cancelled&&!DownIofo.网络超时)
                    {
                        ...
                        if (!DownIofo.播放状态 && DownIofo.是否是播放任务)
                        {
                            ...
                            up.UploadVideo(ref DownIofo, DownIofo.文件保存路径, $"{DownIofo.主播名称}_{DownIofo.房间_频道号}/{MMPU.Unix转换为DateTime(DownIofo.开始时间.ToString()).ToString("yyyyMMdd")}_{DownIofo.标题}/");
                            return;
                        }
